...
PROCESS04_A1 = os.path.join(C10918_PROCESS04, 'A1.JPG')
PROCESS04_B1 = os.path.join(C10918_PROCESS04, 'B1.JPG')
PROCESS04_B2 = os.path.join(C10918_PROCESS04, 'B2.JPG')
PROCESS04_C1 = os.path.join(C10918_PROCESS04, 'C1.JPG')
PROCESS04_C2 = os.path.join(C10918_PROCESS04, 'C2.JPG')
PROCESS04_E1 = os.path.join(C10918_PROCESS04, 'E1.JPG')
PROCESS04_E2 = os.path.join(C10918_PROCESS04, 'E2.JPG')
PROCESS04_A1_REF = os.path.join(C10918_PROCESS04, 'A1.TXT')
PROCESS04_B1_REF = os.path.join(C10918_PROCESS04, 'B1.TXT')
PROCESS04_B2_REF = os.path.join(C10918_PROCESS04, 'B2.TXT')
PROCESS04_C1_REF = os.path.join(C10918_PROCESS04, 'C1.TXT')
PROCESS04_C2_REF = os.path.join(C10918_PROCESS04, 'C2.TXT')
PROCESS04_E1_REF = os.path.join(C10918_PROCESS04, 'E1.TXT')
PROCESS04_E2_REF = os.path.join(C10918_PROCESS04, 'E2.TXT')
PROCESS04_DREF_A8  = os.path.join(C10918_PROCESS04, 'DREF_A8.DCT')
PROCESS04_DREF_B8  = os.path.join(C10918_PROCESS04, 'DREF_B8.DCT')
PROCESS04_DREF_C8  = os.path.join(C10918_PROCESS04, 'DREF_C8.DCT')
PROCESS04_DREF_D8  = os.path.join(C10918_PROCESS04, 'DREF_D8.DCT')


class TestJPEGProcess04_Parse(object):
    """JPEG 10918-2 compliance tests for parsing Process 4.
    ...
    def test_all_required_markers_found(self):
        ...
        optional_keys = ['RST0', 'RST1', 'RST2', 'RST3', 'RST4', 'RST5',
                         ...
                         'APP13', 'APP14', 'APP15']
        for info in [parse_jpg(open(PROCESS04_A1, 'rb')),  parse_jpg(open(PROCESS04_B2, 'rb'))]:
        ...
        info = parse_jpg(open(PROCESS04_B1, 'rb'))
        keys = sorted(info.keys(), key=lambda x: int(x.split('@')[1]))
        keys = [kk.split('@')[0] for kk in keys]
        # Markers with (G) must be present
        for marker in required_keys:
    ...
    def test_parse_a1(self):
        ...
        with open(PROCESS04_A1, 'rb') as fp:
            info = parse_jpg(fp)
            file_length = fp.tell()
    ...
    def test_parse_b1(self):
        ...
        with open(tempfile.name, 'r') as tfile:
            with open(PROCESS04_B1_REF, 'r', encoding='utf-8', errors='ignore') as rfile:
                for out, ref in zip(tfile, rfile):
                    assert ref == out

    def test_parse_b2(self):
        """Test that parsing the B2 file produces the correct output."""
        with open(PROCESS04_B2, 'rb') as fp:
            info = parse_jpg(fp)
            file_length = fp.tell()
        ...
        with open(tempfile.name, 'r') as tfile:
            with open(PROCESS04_B2_REF, 'r', encoding='utf-8', errors='ignore') as rfile:
                for out, ref in zip(tfile, rfile):
                    assert ref == out
    ...
    def test_parse_c1(self):
        ...
        with open(PROCESS04_C1, 'rb') as fp:
            info = parse_jpg(fp)
            file_length = fp.tell()
        ...
        with open(tempfile.name, 'r') as tfile:
            with open(PROCESS04_C1_REF, 'r', encoding='utf-8', errors='ignore') as rfile:
                for out, ref in zip(tfile, rfile):
                    assert ref == out
    ...
    def test_parse_c2(self):
        ...
        with open(PROCESS04_C2, 'rb') as fp:
            info = parse_jpg(fp)
            file_length = fp.tell()
        ...
        with open(tempfile.name, 'r') as tfile:
            with open(PROCESS04_C2_REF, 'r', encoding='utf-8', errors='ignore') as rfile:
                for out, ref in zip(tfile, rfile):
                    assert ref == out

    def test_parse_e1(self):
        """Test that parsing the E1 file produces the correct output."""
        COMMON.WRITE_SOS_DATA = True

        with open(PROCESS04_E1, 'rb') as fp:
            info = parse_jpg(fp)
            file_length = fp.tell()
        ...
        with open(tempfile.name, 'w') as tfile:
            tfile.write(
                "Parser output for 'teste1.jpg', May 9, 1994, "
                "WBP and JLM (IBM)\n"
            )
            # Data
            for key in keys:
        ...
        with open(tempfile.name, 'r') as tfile:
            with open(PROCESS04_E1_REF, 'r', encoding='utf-8', errors='ignore') as rfile:
                for out, ref in zip(tfile, rfile):
                    assert ref == out

    def test_parse_e2(self):
        """Test that parsing the E2 file produces the correct output."""
        COMMON.WRITE_SOS_DATA = True

        with open(PROCESS04_E2, 'rb') as fp:
            info = parse_jpg(fp)
            file_length = fp.tell()
        ...
        with open(tempfile.name, 'w') as tfile:
            tfile.write(
                "Parser output for 'teste2.jpg', May 9, 1994, "
                "WBP and JLM (IBM)\n"
            )
            # Data
            for key in keys:
        ...
        with open(tempfile.name, 'r') as tfile:
            with open(PROCESS04_E2_REF, 'r', encoding='utf-8', errors='ignore') as rfile:
                for out, ref in zip(tfile, rfile):
                    assert ref == out
